...
- STM32 上报的髋关节和小腿角已经是世界坐标系 absolute 角，bridge 不再叠加仿真 offset。
- `/robot_state` 中 `hip_absolute`、`calf_absolute` 与实物世界角一致。
- VMC 输出到电机后的力矩方向正确。
- 电机最大力矩和变化率不比仿真基线更激进。
...
- 若出现“仅单侧在某一方向或接近 `phi_target` 时更容易高频抖动”，不要只检查力矩极性；应同时记录该侧 `phi_rate`、髋膝关节速度和 `/joint_command`，优先排查目标附近闭环阻尼、单边速度状态过大、速度估计噪声、执行死区、机械回差或线束干涉。
