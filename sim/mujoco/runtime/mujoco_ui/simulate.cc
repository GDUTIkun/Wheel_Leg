...
bool SavePpmImage(const std::string& path, const unsigned char* rgb,
  ...
  output << "P6\n" << width << " " << height << "\n255\n";
  output.write(reinterpret_cast<const char*>(rgb), 3 * width * height);
  return output.good();
}
...
void UpdateWatch(mj::Simulate* sim, const mjModel* m, const mjData* d) {
  ...
  // MinGW can fail to compile MJDATA_POINTERS bound expressions from mjxmacro.h
  // because some dimensions expand to bare identifiers instead of model members.
  // For the watch UI, a null check is sufficient to keep the feature usable.
  #define X(TYPE, NAME, NR, NC)                                                                  \
    if (!mju::strcmp_arr(#NAME, sim->field) &&                                                   \
        !mju::strcmp_arr(#TYPE, "mjtNum")) {                                                     \
      if (sim->index >= 0 && d->NAME) {                                                          \
...
void Simulate::Render() {
  ...
  if (this->screenshotrequest.exchange(false)) {
    ...
    // save as PPM to keep this local build self-contained.
    // TODO(b/241577466): Parse the stem of the filename and use a matching extension.
    ...
    // filename for now.
    const std::string path = GetSavePath("screenshot.ppm");
    if (!path.empty()) {
      if (!SavePpmImage(path, rgb.get(), w, h)) {
