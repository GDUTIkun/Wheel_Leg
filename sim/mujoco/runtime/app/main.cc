...
std::string ResolveDefaultModelPath() {
  ...
  const std::vector<std::string> candidates = {
      executable_dir + sep + ".." + sep + ".." + sep + ".." + sep + "sim" + sep +
          "mujoco" + sep + "scenes" + sep + "scence.xml",
      executable_dir + sep + ".." + sep + ".." + sep + ".." + sep + ".." + sep +
          "sim" + sep + "mujoco" + sep + "scenes" + sep + "scence.xml",
      "../scenes/scence.xml",
      "../../scenes/scence.xml",
      "../../sim/mujoco/scenes/scence.xml",
      "../../../sim/mujoco/scenes/scence.xml",
  };
  ...
  return "../../../sim/mujoco/scenes/scence.xml";
}
...
void PhysicsLoop(mj::Simulate& sim) {
  ...
  while (!sim.exitrequest.load()) {
    if (wheel_leg::ShouldRequestRos2BridgeExit()) {
      ...
      if (m) {
        ...
        if (sim.run) {
          ...
          if (elapsedSim < 0 || elapsedCPU.count() < 0 || syncCPU.time_since_epoch().count() == 0 ||
            ...
            wheel_leg::BeforeStep(m, d);
            // run single step, let next iteration deal with timing
            mj_step(m, d);
            wheel_leg::AfterStep(m, d);
...
int main(int argc, char** argv) {
  ...
  wheel_leg::SetSimulateHandle(sim.get());
  ...
  if (argc >  1) {
    ...
    default_filename = ResolveDefaultModelPath();
