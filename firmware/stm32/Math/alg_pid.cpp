...
void Class_PID::TIM_Calculate_PeriodElapsedCallback()
{
    ...
    // 微分滤波(采集微分时, 滤除高频噪声)
    if (Derivative_LPF_RC > 0.0f)
    {
        d_out = d_out * D_T / (Derivative_LPF_RC + D_T) +
                Pre_D_Out * Derivative_LPF_RC / (Derivative_LPF_RC + D_T);
    }
    ...
    // 输出滤波
    if (Output_LPF_RC > 0.0f)
    {
        Out = Out * D_T / (Output_LPF_RC + D_T) +
              Pre_Out * Output_LPF_RC / (Output_LPF_RC + D_T);
    }
