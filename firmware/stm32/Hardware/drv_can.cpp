...
void can_filter_mask_config(FDCAN_HandleTypeDef *hfdcan)
{
    ...
    // 过滤器0: 对应旧CAN规则 (0x201, mask 0x07FC)
    can_filter_init_structure.FilterIndex = 0;
    can_filter_init_structure.FilterID1 = 0x201;
    can_filter_init_structure.FilterID2 = 0x07FC;
    HAL_FDCAN_ConfigFilter(hfdcan, &can_filter_init_structure);

    // 过滤器1: 对应旧CAN规则 (0x029, mask 0x70F)
    can_filter_init_structure.FilterIndex = 1;
    can_filter_init_structure.FilterID1 = 0x029;
    can_filter_init_structure.FilterID2 = 0x70F;
    HAL_FDCAN_ConfigFilter(hfdcan, &can_filter_init_structure);

    // 过滤器2: 对应GIM反馈ID组 (0x03C, mask 0x70F), 可覆盖0x3C/0x5C/0x7C/0x9C
    can_filter_init_structure.FilterIndex = 2;
    can_filter_init_structure.FilterID1 = 0x03C;
    can_filter_init_structure.FilterID2 = 0x70F;
    HAL_FDCAN_ConfigFilter(hfdcan, &can_filter_init_structure);

    // 全局过滤: 未命中的标准/扩展数据帧全部拒绝
...
//            CAN1_Manage_Object.Rx_Timestamp = SYS_Timestamp.Get_Current_Timestamp();
            ...
            if (CAN1_Manage_Object.Callback_Function != 0)
            {
