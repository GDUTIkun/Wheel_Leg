...
| 控制算法调参与行为收敛任务 | `doc/tasks/control_tuning.md` | `[~] 进行中` | 已完成观测链路收口与 `velocity` 回中参考漂移修复，下一步优先做站立平衡点校准 |
...
6. `[ ]` 优先补充 `phi/pitch` 观测并校准站立平衡点。
7. `[ ]` 最后回到速度、转向和高度参数调优，并记录新的基线。
...
- 当前已完成真实遥控器到仿真控制链联调，`stand`、`velocity`、failsafe 与恢复复控能力已形成可调参基线。
...
- 当前下一步继续观察纯积分 `x_ref` 下的速度持续性、超调和稳态振荡。

### 7.3 输入与滤波问题

- 当前输入滤波参数偏小，速度命令平滑效果不足，后续主要调参入口先固定为 `velocity_ref_lpf_rc`，必要时再联动 `velocity_ref_slew_rate`。
- 当前文档先固定默认实现口径：`velocity_ref_lpf_rc` 越大，滤波越强；`velocity_ref_slew_rate` 越小，速度参考变化越平缓。
- 当前文档先固定 `0.1 m/s` 的速度参考 snap 规则：当 `|filtered_target_velocity - commanded_target_velocity| <= 0.1` 时，速度参考直接贴到目标。
- 当前已确认 `/cmd_vel.linear.x` 回中为 `0.0`，下一阶段暂不调整 `linear_x.center`、`linear_x.deadzone`、`scale/reverse` 等遥控输入参数。
...
- 当前现场判断单纯增大速度权重效果不明显，下一步优先验证纯积分 `x_ref` 后是否减少“追误差-超调-反向追”的振荡。
